---
title: "Building a Software Rasterizer from Scratch"
date: 2026-07-30
draft: false
description: "Breaking down how a software rasterizer works, from the color buffer to perspective projection, meshes, and line drawing algorithms"
---

Creating a software rasterizer is, imo, one of the best ways to actually learn computer graphics. Especially in C, where there is little to no hand holding. There are some really good resources out there like <a href="https://github.com/ssloy/tinyrenderer">tinyrenderer</a> (in C++) and <a href="https://gabrielgambetta.com/computer-graphics-from-scratch/">Gabriel Gambetta's graphics programming series</a>, and i've gone through pretty much all of them. This is my attempt to break each part down as much as i can
...
A 2D vector is just a point or direction in 2D space, with an `x` and a `y` component:
...
A 3D vector adds a `z` component for depth:
...
with these we can do basic operations like
...
This works for certain use cases (like 2D games or CAD tools) but it has no sense of depth. A cube drawn orthographically looks flat. so stuff doesnt get smaller or bigger depending on your pov.
...
The idea behind perspective is quite simple: things that are farther away should appear smaller. We achieve this with the perspective divide:
...
In a **left-handed coordinate system**, the same trick with your left hand means Z points into the screen, away from the viewer. DirectX uses left-handed coordinates.
...
A **mesh** is basically a collection of triangles arranged in a 3D space to give the impression of a solid object. Why triangles specifically though, right, could be anything? Because three points always define a flat plane. Four points might not be coplanar (think of something like a wobbly table), but three always are.

Each triangle comes with 3 vertices. A single face of a cube is just 2 triangles sharing an edge if you think about it, giving you 4 unique vertices total. A full cube is 6 faces, so 12 triangles, 8 unique vertices. and thats all a mesh is, a collection of these triangles trying to form a particular object or shape or anything.
...
And then we come into rasterizing these lines. There are 2 common algorithms for actually rasterizing a line onto a pixel grid: **DDA** and **Bresenham's line algorithm**. DDA is simpler to understand but Bresenham is faster. I used DDA in mine, well actually there were a lot of performance tradeoffs but this is for learning and its done on cpu not gpu. We are not trying to make it fast anyway, because if we were we would actually be writing a 3d renderer in gpu with proper graphics apis.
...
The problem with DDA is that it uses floating point arithmetic the whole way through. Every step involves a float increment and a `round()` call. That adds up if we use models that use a lot of lines or need a lot of lines, so something other than low poly gets stuck or hangs or whatever.
...
This is likely part 1 of this and it wont make sense to anyone except me who will read this in 2 weeks. Here is the code if anyone ever reads this:

<a href="https://github.com/loveucifer/gevurah">gevurah</a>
